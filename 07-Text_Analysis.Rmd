...
length(tokens)                    # sanity-check: ~25 000 tokens
```
```r
12322
```
...
vocab <- create_vocabulary(it) %>% prune_vocabulary(term_count_min = 5) # build + prune
```       
...
```r
sim_words("dorothy")
scarecrow        oz   replied 
0.7104297 0.6610949 0.6609231 
```
